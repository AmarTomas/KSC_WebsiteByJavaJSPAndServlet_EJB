<%@page import="np.DeActivitysServlet"%>
...
<html>
   ...
   <jsp:include page="../Nav/navigation.jsp" />
        <div class="card-container">
            <%
                 DeActivitysServlet deActivitysServlet= new DeActivitysServlet();
                      String userId = (String) session.getAttribute("id").toString();
                List<Card> cardList = deActivitysServlet.getAllActivitys_user_id(userId);// Call the servlet function to retrieve card data
    ...
    <div class="card" onclick="viewCardDetails('<%= card.getId() %>')">
        ...
        <h2 ><%= card.getName() %></h2>
<!--        <p>ID: <%= card.getId() %></p>-->
        <p>Fees_details: $<%= card.getFees_details() %></p>
    </div>
    <% 
           }
       } else {
    ...
    <script>
        function viewCardDetails(cardId) {
            window.location.href = "../home/card_details.jsp?id=" + cardId;
        }
    </script>
   <style>
        .card-container {
